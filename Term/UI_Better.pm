package Term::UI_Better;
$Term::UI_Better::VERSION = '0.50';
...
use Term::UI_Better::History_Better;
...
Term::UI_Better - Term::ReadLine UI made easy
...
=head1 SYNOPSIS
    ...
    ### don't have Term::UI_Better issue warnings -- default is '1'
    $Term::UI_Better::VERBOSE = 0;

    ### always pick the default (good for non-interactive terms)
    ### -- default is '0'
    $Term::UI_Better::AUTOREPLY = 1;

    ### Retrieve the entire session as a printable string:
    $hist = Term::UI_Better::History->history_as_string;
    $hist = $term->history_as_string;

=head1 DESCRIPTION

C<Term::UI_Better> is a transparent way of eliminating the overhead of having
...
the user can pick from and a default and C<Term::UI_Better> will DWYM.

For asking a yes or no question, there's even a shortcut.

=head1 HOW IT WORKS

C<Term::UI_Better> places itself at the back of the C<Term::ReadLine>
C<@ISA> array, so you can call its functions through your term object.

C<Term::UI_Better> uses C<Term::UI_Better::History> to record all interactions
with the commandline. You can retrieve this history, or alter
the filehandle the interaction is printed to. See the
C<Term::UI_Better::History> manpage or the C<SYNOPSIS> for details.
...
C<< Term::UI_Better->history_as_string >> function.
...
Convenience wrapper around C<< Term::UI_Better::History->history_as_string >>.

Consult the C<Term::UI_Better::History> man page for details.

=cut

sub history_as_string { return Term::UI_Better::History->history_as_string };

1;

=head1 GLOBAL VARIABLES

The behaviour of Term::UI_Better can be altered by changing the following
global variables:

=head2 $Term::UI_Better::VERBOSE

This controls whether Term::UI_Better will issue warnings and explanations
as to why certain things may have failed. If you set it to 0,
Term::UI_Better will not output any warnings.
The default is 1;

=head2 $Term::UI_Better::AUTOREPLY
...
=head2 $Term::UI_Better::History::HISTORY_FH

This is the filehandle all the print statements from this module
are being sent to. Please consult the C<Term::UI_Better::History> manpage
for details.
...
C<$reply> will hold one of the choices presented. C<Term::UI_Better> will repose
the question if the user attempts to enter an answer that's not in the
list of choices. The string presented is held in the C<$Term::UI_Better::INVALID>
...
(or set C<$Term::UI_Better::AUTOREPLY> -- see the C<GLOBAL VARIABLES> section) and
...
In this case, simply setting C<$Term::UI_Better::AUTOREPLY> to true, will
return from every question with the default answer set for the question.
Do note that if C<AUTOREPLY> is true, and no default is set, C<Term::UI_Better>
will warn about this and return C<undef>.

=head1 See Also

C<Params::Check>, C<Term::ReadLine>, C<Term::UI_Better::History>
